...
if __name__ == "__main__":
    ...
    while food_counter<3000:
        ...
        if rank != 0:
            ...
            food_counter = comm.recv(source=1)
            ants.directions = comm.recv(source=1)
            ants.historic_path = comm.recv(source=1)
            ants.age = comm.recv(source=1)
            pherom.pheromon = comm.recv(source=1)
    ...
    fin = time.time()
    print("Temps d'exécution : ",fin - ref)
